  ...
  return (
    <div className="w-full">
        ...
        <div className="max-w-[1440px] mx-auto py-4 px-10 flex-col justify-between text-center md:flex-row">
          ...
          <h1 className='text-purple-500 underline text-l'>{user.email}</h1> <hr /><br />
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
            {userAlbums.map(({id, title }) => <div key={id}>
              <Link to={`/album/${`album`}`}><h3 className=' font-semibold text-center'>{title}</h3></Link>
              <div>{photos.map(({id, title, thumbnailUrl}) => <div key={id}> 
                  <h4>{title}</h4>
